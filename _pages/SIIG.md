---
layout: page
permalink: /siig/
title: SIIG
nav_title: group
description: Spatial Intelligent and Interaction Group.
nav: true
nav_order: 2
---



## About

<img src="/assets/img/siig.png" alt="SIIG Logo" style="float:right; height:6rem; max-width:12rem; margin:-10rem 1rem 1rem 1rem;" />

The **SIIG (Spatial Intelligent and Interaction Group)** is a research group under [Zhejiang Provincial Key Laboratory of BCCITA](https://research.hdu.edu.cn/2026/0311/c11945a290176/page.htm) and [Institute of Cognitive Computing and Intelligent Interaction](https://cs.hdu.edu.cn/12277/list.htm). 

Our primary research areas focus on the intersection of spatial intelligence and embodied intelligence. We approach problems through three guiding principles—**unified** systems, **generalized** methods, and **robust** performance. Key topics include SLAM, localization, reconstruction, representation, interaction, and related fields. An overview of these directions and core results can be found in the figure below.

我们的主要研究方向聚焦空间智能与具身智能的交叉，围绕系统的**统一性**、**通用性**和**鲁棒性**这三个方面开展研究工作。主要研究方向包括 SLAM、定位、重建、表示、交互以及其他相关方向。方向汇总与核心成果可在下图中查看。

<img src="/assets/img/SIIG_Research_Overview_with_paper.png" alt="SIIG Research Overview" style="width:100%; max-width:760px; height:auto;" />



<!-- ### Research Areas

- Computer vision for autonomous robots  
- Deep learning for manipulation  
- Human–robot collaboration  
…etc. -->

### Supporting Projects
 1. 基于共享多任务低维隐码的空间人工智能系统空间感知能力研究，*国自然青年项目*，2025-2027【<span style="color:red;">统一</span>、<span style="color:green;">通用</span>】；
 2. 具备终生学习能力的机器人通用视觉自主定位技术研究，*浙江省自然青年项目*，2025-2026【<span style="color:green;">通用</span>、<span style="color:blue;">鲁棒</span>】；
 3. 视觉基础模型驱动的 3DGS 通用场景建模与多任务适配研究，*CAD&CG全重实验室开放课题*，2025-2026【<span style="color:red;">统一</span>、<span style="color:green;">通用</span>】；
 4. 面向封闭低光照环境的视-惯-热融合高斯溅射SLAM算法研究，*ICT全重实验室开放课题*，2025【<span style="color:blue;">鲁棒</span>】；
 5. 医疗急救无人机在亚运医疗保障的应用，*杭州亚运会智能应用项目*，2022【<span style="color:blue;">鲁棒</span>】。


### People

- Principal Investigator: Yuhang Ming
<!-- - Dr. Jane Doe – Postdoctoral fellow   -->
<!-- - PhD students: Alice, Bob, …   -->
- Master Students: Liangjun Huang (2026), Junyi Mao (2026), Wenjun Wang (2026), Size Song (2026), Ming Liu (2026), Chenxin Fang (2025), Meiyue Guo (2025), Tingkang Xi (2024), Zhi Zhang (2024).
- Undergraduate Students: Zhengyi Xu (2024), Hanyu Zhu (2023).
- Alumni: 
  - 2026: Di Ma (M.E., HUAWEI, 国家奖学金， 浙江省优秀毕业生) Minyang Xu (M.E., YIQIYIN FINTECH, 国家奖学金， 浙江省优秀毕业生), Jinyi Xu (B.E., Tongji University).
    <div style="text-align:right; margin-top:1rem;">
      <img src="/assets/img/graduate-2026.jpg" alt="class of 2026" style="max-width:90%; height:auto; border-radius:0.5rem;" />
      <p style="margin-top:0.5rem; font-size:0.9rem; color:#666;">Class of 2026</p>
    </div>


### Join Us

We’re always looking for motivated students and collaborators. Contact Dr. Ming at `yuhang.ming(at)hdu(dot)edu(dot)cn`.
